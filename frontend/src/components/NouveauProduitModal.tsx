...
export default function NouveauProduitModal({ onClose, onCreated, onUpdated, product, knownCategories, existingProducts = [], prefill }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    if ((form.category === '__new__' || form.subCategory === '__newsub__') && finalCategory) {
      addCategory(finalCategory, finalSubCategory).catch(() => {});
    }
    setLoading(true);
    setError('');
