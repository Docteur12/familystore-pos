...
const STATUS_CFG: Record<AlertStatus, { label: string; bg: string; color: string }> = {
  rupture:  { label: t('Rupture', 'Out of stock'),  bg: 'var(--fs-wine-100)', color: 'var(--fs-wine-700)' },
  critique: { label: t('Critique', 'Critical'), bg: '#FEF0E0', color: '#8B5A14' },
  alerte:   { label: t('Alerte', 'Alert'),   bg: '#F7ECD4', color: '#8B5A14' },
};
...
export default function StocksAlertes() {
  ...
  const suggestions = products
    ...
    .map(p => ({
      ...
      urgency: p.stock === 0 ? t('Urgent', 'Urgent') : p.stock <= p.alertThreshold * 0.4 ? t('Critique', 'Critical') : t('Normal', 'Normal'),
    }))
  ...
  const exportData = (): { title: string; headers: string[]; rows: (string | number)[][] } => {
    if (tab === 'peremption') {
      return {
        title: t('Péremption proche', 'Near expiry'),
        headers: [t('Produit', 'Product'), t('Catégorie', 'Category'), t('Jours restants', 'Days left'), t('Date péremption', 'Expiry date')],
        rows: expiryProducts.map(({ p, days }) => [p.name, p.category ?? '', days, expiryOf(p)?.toLocaleDateString(dateLocale()) ?? '']),
      };
    }
    if (tab === 'suggestions') {
      return {
        title: t('Suggestions de réapprovisionnement', 'Restocking suggestions'),
        headers: [t('Produit', 'Product'), t('Stock', 'Stock'), t('Qté conseillée', 'Suggested qty'), t('Urgence', 'Urgency')],
        rows: suggestions.map(({ p, recommended, urgency }) => [p.name, p.stock, recommended, urgency]),
      };
    }
    return {
      title: t('Réapprovisionnement', 'Restocking'),
      headers: [t('Produit', 'Product'), t('Catégorie', 'Category'), t('Stock actuel', 'Current stock'), t('Seuil', 'Threshold'), t('Statut', 'Status')],
  ...
  const handleExportPdf = async () => {
    ...
    if (rows.length === 0) { addToast(t('Rien à exporter', 'Nothing to export'), 'error'); return; }
    try {
      ...
      doc.text(`${nomMagasin} — ${title}`, margin, 12);
      doc.setFont('helvetica', 'normal'); doc.setFontSize(9);
      doc.text(t(`${rows.length} produit(s)  ·  ${new Date().toLocaleDateString(dateLocale())}`, `${rows.length} product(s)  ·  ${new Date().toLocaleDateString(dateLocale())}`), margin, 19);
      let y = 34;
      const head = () => {
  ...
  const tabs: { id: TabKey; label: string; count: number; color: string }[] = [
    { id: 'reappro',     label: t('Réapprovisionnement', 'Restocking'), count: lowProducts.length,     color: '#D97706' },
    { id: 'peremption',  label: t('Péremption proche', 'Near expiry'),   count: expiryProducts.length,  color: '#DC2626' },
    { id: 'suggestions', label: t('Suggestions auto', 'Auto suggestions'),    count: suggestions.length,     color: 'var(--fs-wine-700)' },
  ];
    ...
    <div style={{ display: 'flex', width: '100vw', height: '100vh', overflow: 'hidden', position: 'fixed', top: 0, left: 0, fontFamily: 'var(--fs-font-sans)' }}>
      ...
      <main style={{ flex: 1, display: 'flex', flexDirection: 'column', overflowX: 'hidden', overflowY: 'auto', background: 'var(--fs-ivory)' }}>
        ...
        <div style={{ background: '#fff', borderBottom: '1px solid var(--fs-line)', padding: isNarrow ? '12px 16px' : '12px 24px', flexShrink: 0 }}>
          <div style={{ display: 'flex', flexDirection: isNarrow ? 'column' : 'row', alignItems: isNarrow ? 'stretch' : 'center', justifyContent: 'space-between', gap: isNarrow ? 10 : 16 }}>
            <div style={{ paddingLeft: isMobile ? 44 : 0 }}>
              ...
              <h1 style={{ fontSize: 20, fontWeight: 800, color: 'var(--fs-ink-900)', margin: 0 }}>{t('Alertes & Seuils', 'Alerts & Thresholds')}</h1>
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              <button onClick={handleEmailRecap}
                ...
                <I d={D.mail} size={13}/> {t('Récap email', 'Email recap')}
              </button>
              <button onClick={handleExportPdf} title={t('Exporter en PDF (onglet actif)', 'Export to PDF (active tab)')}
              ...
              <button onClick={handleExportExcel} title={t('Exporter en Excel (onglet actif)', 'Export to Excel (active tab)')}
                style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', border: '1.5px solid var(--fs-wine-700)', borderRadius: 8, background: '#fff', cursor: 'pointer', color: 'var(--fs-wine-700)', fontSize: 13, fontWeight: 600, fontFamily: 'var(--fs-font-sans)' }}>
                Excel
              </button>
              <button onClick={load}
                style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', border: '1.5px solid var(--fs-line-2)', borderRadius: 8, background: '#fff', cursor: 'pointer', color: 'var(--fs-ink-500)', fontSize: 13, fontWeight: 600, fontFamily: 'var(--fs-font-sans)' }}>
                <I d={D.refresh} size={13}/> {t('Actualiser', 'Refresh')}
              </button>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: isNarrow ? 'grid' : 'flex', gridTemplateColumns: isNarrow ? '1fr 1fr' : undefined, gap: isNarrow ? 10 : 14, padding: isNarrow ? '14px 16px' : '16px 24px', flexShrink: 0 }}>
          {([
            { label: t('Ruptures', 'Out of stock'),  count: ruptures,  bg: 'var(--fs-wine-100)', color: 'var(--fs-wine-700)', filter: 'rupture' },
            { label: t('Critiques', 'Critical'), count: critiques, bg: '#FEF0E0', color: '#8B5A14', filter: 'critique' },
            { label: t('Alertes', 'Alerts'),   count: alertes,   bg: '#F7ECD4', color: '#8B5A14', filter: 'alerte' },
            { label: t('Total', 'Total'),     count: alertCount, bg: 'var(--fs-wine-50)', color: 'var(--fs-wine-800)', filter: 'all' },
            ...
            <div key={s.label} onClick={() => { setTab('reappro'); setStatusFilter(s.filter); }}
              title={t(`Voir : ${s.label}`, `View: ${s.label}`)}
          ...
          {loading ? (
            ...
            displayedLow.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '80px', color: 'var(--fs-ink-300)' }}>
                ...
                <div style={{ fontSize: 15, fontWeight: 600 }}>{statusFilter === 'all' ? t('Tous les stocks sont suffisants', 'All stock levels are sufficient') : t(`Aucun produit « ${STATUS_CFG[statusFilter].label} »`, `No "${STATUS_CFG[statusFilter].label}" products`)}</div>
                {statusFilter !== 'all' && <button onClick={() => setStatusFilter('all')} style={{ marginTop: 12, padding: '6px 14px', border: '1.5px solid var(--fs-line-2)', borderRadius: 8, background: '#fff', cursor: 'pointer', fontSize: 12, fontWeight: 600, color: 'var(--fs-ink-500)' }}>{t('Voir tout', 'View all')}</button>}
              </div>
            ) : (
              ...
              <table className="fs-grid" style={{ width: '100%', minWidth: isNarrow ? 720 : 640, borderCollapse: 'collapse' }}>
                <thead>
                  <tr>
                    {[t('Produit', 'Product'), t('Catégorie', 'Category'), t('Stock actuel', 'Current stock'), t('Seuil (auto 10%)', 'Threshold (auto 10%)'), t('Statut', 'Status')].map((h, i) => (
                    ...
                    return (
                      <tr key={p._id} style={{ background: idx % 2 === 0 ? '#fff' : 'var(--fs-ivory)', borderBottom: '1px solid var(--fs-line)' }}>
                        ...
                        <td style={{ padding: '10px 12px', textAlign: 'center' }}>
                          ...
                          <div style={{ fontSize: 10, color: 'var(--fs-ink-400)' }}>{t('= 10% de', '= 10% of')} {p.initialStock ?? p.alertThreshold * 10}</div>
                        </td>
                        <td style={{ padding: '10px 12px' }}>
            ...
            expiryProducts.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '80px', color: 'var(--fs-ink-300)' }}>
                <div style={{ fontSize: 36, marginBottom: 10 }}>✓</div>
                <div style={{ fontSize: 15, fontWeight: 600 }}>{t(`Aucune péremption dans les ${maxDays} jours`, `No expiry within ${maxDays} days`)}</div>
              </div>
            ) : (
              <div style={{ overflowX: 'auto' }}>
              <table className="fs-grid" style={{ width: '100%', minWidth: isNarrow ? 720 : 640, borderCollapse: 'collapse' }}>
                <thead>
                  <tr>
                    {[t('Produit', 'Product'), t('Catégorie', 'Category'), t('Stock', 'Stock'), t('Date péremption', 'Expiry date'), t('Délai', 'Time left'), t('Urgence', 'Urgency')].map((h, i) => (
            ...
            suggestions.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '80px', color: 'var(--fs-ink-300)' }}>
                <div style={{ fontSize: 36, marginBottom: 10 }}>✓</div>
                <div style={{ fontSize: 15, fontWeight: 600 }}>{t('Aucune suggestion de commande', 'No order suggestions')}</div>
              </div>
            ) : (
              <>
                <div style={{ background: '#fff', border: '1px solid rgba(122,29,46,0.15)', borderRadius: 10, padding: '12px 16px', marginBottom: 14, fontSize: 12, color: 'var(--fs-wine-800)', display: 'flex', alignItems: 'center', gap: 8 }}>
                  <I d={D.zap} size={14}/> {t('Ces suggestions sont calculées automatiquement : stock conseillé = 2× seuil d\'alerte.', 'These suggestions are calculated automatically: recommended stock = 2× alert threshold.')}
                </div>
                <div style={{ overflowX: 'auto' }}>
                <table className="fs-grid" style={{ width: '100%', minWidth: isNarrow ? 720 : 640, borderCollapse: 'collapse' }}>
                  <thead>
                    <tr>
                      {[t('Produit', 'Product'), t('Catégorie', 'Category'), t('Stock actuel', 'Current stock'), t('Seuil', 'Threshold'), t('Qté recommandée', 'Recommended qty'), t('Urgence', 'Urgency')].map((h, i) => (
                        <th key={h} style={{ padding: '10px 12px', textAlign: i >= 2 ? 'center' : 'left', fontSize: 10, fontWeight: 700, color: 'var(--fs-ink-400)', textTransform: 'uppercase', letterSpacing: '0.1em', borderBottom: '1px solid var(--fs-line)', position: 'sticky', top: 0, background: '#fff', zIndex: 1 }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                      ...
                      <tr key={p._id} style={{ background: idx % 2 === 0 ? '#fff' : 'var(--fs-ivory)', borderBottom: '1px solid var(--fs-line)' }}>
                        ...
                        <td style={{ padding: '10px 12px', textAlign: 'center' }}>
                          <span style={{
                            ...
                            background: urgency === 'Urgent' ? 'var(--fs-wine-100)' : urgency === t('Critique', 'Critical') ? '#FEF0E0' : '#F7ECD4',
