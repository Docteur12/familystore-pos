...
const TYPE_CFG: Record<string, { label: string; bg: string; color: string; icon: string }> = {
  vente:        { label: t('Vente', 'Sale'),        bg: '#E8F0E5', color: '#3F6B3A', icon: 'M6 3h12v18l-3-2-3 2-3-2-3 2V3zM9 8h6M9 12h6M9 16h4' },
  connexion:    { label: t('Connexion', 'Login'),    bg: '#EEF3FA', color: '#3A5E8F', icon: 'M15 3h4a2 2 0 012 2v14a2 2 0 01-2 2h-4M10 17l5-5-5-5M15 12H3' },
  creation:     { label: t('Création', 'Creation'),     bg: '#E8F5E8', color: '#2E7D2E', icon: 'M12 5v14M5 12h14' },
  modification: { label: t('Modification', 'Update'), bg: '#F0EAF7', color: '#6B3FA0', icon: 'M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z' },
  suppression:  { label: t('Suppression', 'Deletion'),  bg: 'var(--fs-wine-100)', color: 'var(--fs-wine-700)', icon: 'M3 6h18M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a1 1 0 011-1h4a1 1 0 011 1v2' },
};

const MODULE_CFG: Record<string, { label: string; color: string }> = {
  auth:          { label: 'Auth',         color: '#3A5E8F' },
  utilisateurs:  { label: t('Utilisateurs', 'Users'), color: '#6B3FA0' },
  produits:      { label: t('Produits', 'Products'),     color: '#8B5A14' },
  ventes:        { label: t('Ventes', 'Sales'),       color: '#3F6B3A' },
  stock:         { label: 'Stock',        color: '#1D7A4E' },
  caisses:       { label: t('Caisses', 'Registers'),      color: 'var(--fs-wine-700)' },
  'paramètres':  { label: t('Paramètres', 'Settings'),   color: '#4A4A6A' },
};
...
// Rôle technique (base) → libellé affiché
...
function exportCSV(entries: AuditLogEntry[]) {
  const header = t('Date;Heure;Utilisateur;Rôle;Type;Module;Détail', 'Date;Time;User;Role;Type;Module;Detail');
...
export default function AdminAudit() {
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (!silent) addToast(t('Erreur chargement des logs', 'Error loading logs'), 'error');
    } finally {
      ...
      <main style={{ flex: 1, display: 'flex', flexDirection: 'column', overflowX: 'hidden', overflowY: isNarrow ? 'auto' : 'hidden', background: 'var(--fs-ivory)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: isNarrow ? 'column' : 'row', alignItems: isNarrow ? 'stretch' : 'center', justifyContent: 'space-between', gap: isNarrow ? 10 : 16 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: 'var(--fs-ink-400)' }}>
                ...
                {t('Mis à jour', 'Updated')} {relTime(lastRefresh.toISOString())}
              </div>
              {/* Search */}
              ...
              >
                <I d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4M7 10l5 5 5-5M12 15V3" size={13}/> {t('Exporter CSV', 'Export CSV')}
              </button>
              {/* Refresh manuel */}
              <button
                onClick={() => load()}
        ...
        <div style={{ display: 'flex', gap: 10, padding: isNarrow ? '12px 16px' : '12px 28px', flexShrink: 0, flexWrap: 'wrap' }}>
          {stats && [
            ...
            { label: t('Ventes', 'Sales'),        value: stats.vente,        bg: '#E8F0E5',   color: '#3F6B3A'           },
            { label: t('Connexions', 'Logins'),    value: stats.connexion,    bg: '#EEF3FA',   color: '#3A5E8F'           },
            { label: t('Créations', 'Creations'),     value: stats.creation,     bg: '#E8F5E8',   color: '#2E7D2E'           },
            { label: t('Modifications', 'Updates'), value: stats.modification, bg: '#F0EAF7',   color: '#6B3FA0'           },
            { label: t('Suppressions', 'Deletions'),  value: stats.suppression,  bg: 'var(--fs-wine-100)',   color: 'var(--fs-wine-700)'           },
          ].map(s => (
        ...
        <div style={{ display: 'flex', gap: 6, padding: isNarrow ? '0 16px 6px' : '0 28px 6px', flexShrink: 0, flexWrap: 'wrap' }}>
          <span style={{ fontSize: 11, color: 'var(--fs-ink-400)', fontWeight: 600, alignSelf: 'center', marginRight: 4 }}>{t('Type :', 'Type:')}</span>
          {ALL_TYPES.map(ty => {
            const active = typeFilter === ty;
            const cfg    = ty !== 'all' ? TYPE_CFG[ty] : null;
            return (
              <button key={ty} onClick={() => setTypeFilter(ty)} style={{
                ...
                {ty === 'all' ? t('Tous', 'All') : TYPE_CFG[ty].label}
              </button>
            );
          })}
        </div>
        ...
        <div style={{ display: 'flex', gap: 6, padding: isNarrow ? '0 16px 12px' : '0 28px 12px', flexShrink: 0, flexWrap: 'wrap' }}>
          <span style={{ fontSize: 11, color: 'var(--fs-ink-400)', fontWeight: 600, alignSelf: 'center', marginRight: 4 }}>{t('Module :', 'Module:')}</span>
          {ALL_MODULES.map(m => {
            ...
            return (
              <button key={m} onClick={() => setModFilter(m)} style={{
                ...
                {m === 'all' ? t('Tous', 'All') : MODULE_CFG[m]?.label ?? m}
              </button>
            );
          })}
        </div>
        ...
        <div style={{ padding: isNarrow ? '0 16px 8px' : '0 28px 8px', fontSize: 11, color: 'var(--fs-ink-400)', flexShrink: 0 }}>
          {filtered.length} {filtered.length !== 1 ? t('entrées', 'entries') : t('entrée', 'entry')}{search || typeFilter !== 'all' || modFilter !== 'all' ? t(' (filtrées)', ' (filtered)') : ''}
        </div>
              ...
              <table className="fs-grid" style={{ width: '100%', borderCollapse: 'collapse', minWidth: isNarrow ? 760 : undefined }}>
                <thead>
                  <tr style={{ background: 'var(--fs-ivory)' }}>
                    {[t('Date / Heure', 'Date / Time'), t('Utilisateur', 'User'), 'Type', 'Module', t('Détail', 'Detail')].map((h, i) => (
                  ...
                  {filtered.length === 0 ? (
                    <tr>
                      <td colSpan={5} style={{ padding: '40px', textAlign: 'center', color: 'var(--fs-ink-400)', fontSize: 13 }}>
                        {entries.length === 0
                          ? t('Aucun événement enregistré — les opérations du système apparaîtront ici en temps réel.', 'No events recorded — system operations will appear here in real time.')
                          : t('Aucune entrée correspondant aux filtres.', 'No entries match the filters.')}
                      </td>
                    </tr>
